...
module.exports = class API {
    ...
    static async fetchCourse_IdByName(req, res) {
        ...
        try {
            const course = await Course.findOne({ name: name }, idProjection(Course), null).exec();
    ...
    static async fetchCourseTrainer(req, res) {
        try {
            const name = req.params.courseName;
            const course = await Course.findOne({ name: name }, idProjection(Course), null).populate('trainer').exec();
            if (!course) {
                return res.status(404).json({ message: 'Course not found' });
            }
    ...
    static async fetchCourseParticipants(req, res) {
        try {
            const name = req.params.courseName;
            const course = await Course.findOne({ name: name }, idProjection(Course), null).populate('participants').exec();
            if (!course) {
                return res.status(404).json({ message: 'Course not found' });
            }
    ...
    static async removeParticipantByUsername(req, res) {
        try {
            ...
            const course = await Course.findOne({ name: courseName }, idProjection(Course), null).populate('participants').exec();
            if (!course) {
