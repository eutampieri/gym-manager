...
<template>
    ...
    <section id="my-courses" class="my-3">
        ...
        <Dropdown id="my-courses-dropdown">
            <DropdownItem v-for="(course, i) in myCourses" :key="i"
                :header="[course.schedule[0].dayOfWeek + ' ' + course.schedule[0].startTime, course.name]"
                ...
                <dl>
                    ...
                    <dt>Trainer</dt>
                    <dd>
                        <NameLink :path="trainerProfilePath(course.trainer)">{{ course.trainer }}</NameLink>
                    </dd>
                </dl>
                <button type="button" class="btn btn-primary m-2"
                    @click="() => unsubscribeFromCourse(course.id, course.name)">Unsubscribe</button>
            </DropdownItem>
        </Dropdown>
    </section>
    <section id="my-one-on-one" class="my-3">
        ...
        <Dropdown id="my-oo-dropdown">
            <DropdownItem v-for="(course, i) in myOneOnOne" :key="i"
                :header="[course.dayOfWeek + ' ' + course.startTime, course.trainer]" :id-prefix="'one-on-one'"
                ...
                <button type="button" class="btn btn-primary m-2" @click="() => cancelSession(course.id)">Cancel
                    appointment</button>
            </DropdownItem>
        </Dropdown>
    </section>
