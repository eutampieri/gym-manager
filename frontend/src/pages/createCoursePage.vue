...
async function fecthTrainerId(trainer) {
    try {
            // FUNZIONE fetchTrainerIdByUsername
            const response = await fetch(`api/trainers/trainerId/${trainer}`);
            if (response.ok) {
                const trainerId = await response.json();
                return trainerId;
            }
            else {
                document.getElementById('message').textContent ='Trainer not found' ;
            }
        } catch (error) {
...
<button class="btn btn-primary" type="button" @click="fecthTrainerId">Create Course {{name }}</button>
